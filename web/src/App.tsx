...
import { loadCheckpointIndex, loadReplay, loadTrainingStatus, resetTraining, runReplay, startTraining } from "./lib/api";
...
export function App() {
  ...
  async function handleClearTraining() {
    ...
    setError(null);
    try {
      const status = await resetTraining();
      setTrainingStatus(status);
      applyCheckpointIndex(null);
    } catch (clearError) {
      setTrainingError(clearError instanceof Error ? clearError.message : "Unable to clear training data.");
    } finally {
